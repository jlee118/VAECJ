...
class VRNNGRUGAN(tf.keras.Model):
	def __init__(self, feature_space, z_dim, latent_dim, timesteps, lambda_gan, vrnn_model, **kwargs):
		...
		# disc_rnn = keras.layers.Bidirectional(keras.layers.GRU(32, recurrent_dropout=0.5, dropout=0.5), merge_mode='ave', name='feature_ext')(disc_input)

		disc_rnn = tfa.layers.SpectralNormalization(keras.layers.Conv1D(filters=12, kernel_size=10, activation='relu'))(disc_input)
		disc_rnn = tf.keras.layers.LeakyReLU(0.2)(disc_rnn)
		# disc_rnn = tfa.layers.SpectralNormalization(keras.layers.Conv1D(filters=32, kernel_size=16, activation='relu'))(disc_input)
		# disc_rnn = tf.keras.layers.LeakyReLU(0.2)(disc_rnn)
		disc_rnn = keras.layers.MaxPooling1D(pool_size=1, strides=1, padding='same')(disc_rnn)
		disc_rnn = keras.layers.Flatten()(disc_rnn)
		disc_rnn = keras.layers.Dense(8)(disc_rnn)
		disc_rnn = tf.keras.layers.LeakyReLU(0.2)(disc_rnn)
		disc_rnn = tf.keras.layers.Dropout(0.2, name='feature_ext')(disc_rnn)
	...
	def train_step(self, data):
		...
		with tf.GradientTape(persistent=True) as tape:
			...
			# discrim_output_loss_real = tf.reduce_mean(
			#     wasserstein_loss(tf.ones_like(discrim_real_output), discrim_real_output)
			# )

			# w_loss = tf.reduce_sum(discrim_fake_output) - tf.reduce_sum(discrim_real_output)
		...
		# discrim_grads,_ = tf.clip_by_global_norm(discrim_grads, 10.0)
		...
		encoder_grads,_ = tf.clip_by_global_norm(encoder_grads, 1.15)
